...
## Purpose
Native zsh-powered command and positional (subcommand/flag/argument) completion for the `!`-triggered shell-autocomplete extension. Queries zsh's completion system via a persistent PTY worker (`ZshWorker`) with sentinel-framed request/response protocol, yielding typical cache-miss latencies of 60–90 ms warm (12–19× faster than the legacy per-query shell spawn). Requirements cover command-list discovery, positional completion, caching, and the persistent-worker infrastructure (bootstrap, sentinel framing, in-flight dedupe, auto-recovery, compdump isolation, latency budget).
## Requirements
...
- **THEN** the worker SHALL be terminated and respawned
